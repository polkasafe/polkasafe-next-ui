...
import { QUICKBOOKS_CSV_HEADERS, XERO_CSV_HEADERS } from '@common/constants/exportTransactionsConstants';
...
const ExportTransactionsHistory = ({ historyTxns, exportType, closeModal }: IExportTransactionsHistory) => {
	...
	const exportToCsv = () => {
		...
		const quickbooksTransactionsCsv = historyTxns.reduce(
			(txns, a, i) => {
				const { date, callhash, token, from, amount } = a;
				const formattedDate = dayjs(date).format('L');
				txns.push(
					[formattedDate, 'General', i + 1, `P-${i + 1}`, callhash, from, '', '', '', '', token, '', amount, ''].join(
						','
					)
				);
				return txns;
		...
		const xeroTransactionsCsv = historyTxns.reduce(
			(txns, a) => {
		...
		downloadFile({
			data:
				exportType === EExportType.QUICKBOOKS
					? [...QUICKBOOKS_CSV_HEADERS, ...quickbooksTransactionsCsv].join('\n')
	...
	return (
		<div className='flex flex-col w-[560px]'>
			{downloaded ? (
				<div className=''>
					...
					<p className='text-white mt-1 ml-1'>
						You can import this into {exportType.charAt(0).toUpperCase() + exportType.slice(1)}.
					</p>
				</div>
			) : (
			...
			<div className='flex items-center justify-between gap-x-5 mt-[30px]'>
				<ActionButtons onCancel={closeModal} onClick={exportToCsv} label='Export' cancelLabel='Close' disabled={downloaded} />
			</div>
		</div>
	);
};
