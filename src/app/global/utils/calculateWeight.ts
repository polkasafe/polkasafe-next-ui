...
export async function calcWeight(call: any, api: ApiPromise): Promise<Result> {
	...
	if (call && api.call.transactionPaymentApi) {
		try {
			const { v1Weight, v2Weight } = convertWeight((await api.tx(call).paymentInfo(ZERO_ACCOUNT)).weight);
			...
			val = objectSpread(
				{},
				{
					...
					v1Weight,
					v2Weight,
					weight: !isFunction(api.registry.createType<V1Weight>('Weight').toBn) ? v2Weight : v1Weight
				}
			);
		} catch (error) {
			console.error(error);
