...
export async function POST(req: Request) {
	...
	const { multisig } = await req.json();
	try {
		const { address: a, disabled, name, network: n, signatories, threshold } = multisig;

		const multisigRef = firestoreDB.collection('multisigAddresses').doc(a);
		...
		return NextResponse.json({ data: null, error: responseMessages.internal }, { status: 200 });
	}
}
