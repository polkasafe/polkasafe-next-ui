...
export async function POST(req: Request) {
	...
	try {
		const addressDoc = await firestoreDB.collection('addresses').doc(docId).get();
		const addressData = addressDoc.data() as IUser;

		return NextResponse.json({ data: addressData.notification_preferences || null }, { status: 200 });
	} catch (err: unknown) {
