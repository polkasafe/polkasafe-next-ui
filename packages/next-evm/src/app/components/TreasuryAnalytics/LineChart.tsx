	...
	useEffect(() => {
		...
		incomingTransactions.forEach((item) => {
			...
			if (
				...
				if (foundData.tokenInfo[item.tokenSymbol]) {
					const tokenInfo = foundData.tokenInfo[item.tokenSymbol];
					tokenInfo.balance_token += Number(item.balance_token);
					tokenInfo.balance_usd += Number(item.balance_usd);
				} else {
					foundData.tokenInfo[item.tokenSymbol] = {
				...
				am5.array.each(series1.dataItems, function (dataItem) {
					if (
						...
						Object.keys(tokenInfo).forEach((token) => {
							// eslint-disable-next-line no-param-reassign
							text += `\n $ ${tokenInfo[token].balance_usd.toFixed(2)} (${tokenInfo[token].balance_token.toFixed(
								2
							)} ${token})`;
						});
					}
