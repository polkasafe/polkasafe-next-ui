...
import getMultisigQueueByAddress from '../api-utils/getMultisigQueueByAddress';
...
export async function POST(req: Request) {
	...
	try {
		...
		if (queueItemsError || !queueItemsArr)
			return NextResponse.json(
				{ data: null, error: queueItemsError || responseMessages.queue_fetch_error },
				{ status: 400 }
			);
		...
		console.error('Error in getMultisigQueue :', { err, stack: (err as any).stack });
