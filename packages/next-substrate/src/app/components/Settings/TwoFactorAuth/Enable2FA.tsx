...
const Enable2FA = ({ className }: { className?: string }) => {
	...
	const handleVerifyTFA = async () => {
		...
		try {
			...
			const { data: verify2FAData, error: verify2FAError } = (await generate2FaRes.json()) as {
				data: IUser;
				error: string;
			};
			...
			<div className='grid grid-cols-10 bg-bg-main rounded-lg p-5 text-white'>
				...
				<div className='col-span-5'>
					<p className='text-text_secondary'>
						Enhance account security with two factor authentication. Verify your identity with an extra step for added
						protection.
					</p>
					<Button
